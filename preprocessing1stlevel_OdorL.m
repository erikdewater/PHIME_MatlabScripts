...
for h=1:numel(dirinfo) %Subjects loop
    ...
    for g=3:3%1:numel(dirinfo_FMRI) %Tasks loop
        FullFilePath_FMRI(numel(dirinfo_FMRI)*(h-1)+g)=strcat(filepath,'/',dirinfo(h).name,'/fMRI/',dirinfo_FMRI(g).name, '/L_OLF/'); % Full file path
...
for h=1:numel(dirinfo) %Subjects loop
    ...
    for j=3:3%numel(dirinfo_FMRI) %Tasks loop 
        FullFilePath_FMRI2=strcat(filepath,'/',dirinfo(h).name,'/fMRI/',dirinfo_FMRI(j).name, '/L_OLF/'); % Full file path
        ...
        dir_model = strcat('model', '_', dirinfo(h).name, '_', dirinfo_FMRI(j).name, '/L_OLF/');
